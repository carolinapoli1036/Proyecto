    ...
    <div style={{ display: 'flex', minHeight: '100vh', flex: 1, fontFamily: sans, flexDirection: 'column' }}>
      ...
      <style>{`
        @media (min-width: 769px) {
          ...
          .login-left { width: 45% !important; display: flex !important; }
          .login-right { padding: 60px !important; }
          .login-mobile-header { display: none !important; }
        }
        @media (max-width: 768px) {
          .login-left { display: none !important; }
          .login-right { padding: 40px 24px !important; align-items: flex-start !important; }
          .login-mobile-header { display: flex !important; }
        }
      `}</style>

      {/* Header solo en móvil */}
      <div className="login-mobile-header" style={{ display: 'none', background: '#1a1a1a', padding: '0 24px', height: '56px', alignItems: 'center', justifyContent: 'space-between' }}>
        <a href="/" style={{ fontSize: '14px', fontWeight: 500, color: '#fff', textDecoration: 'none', fontFamily: sans }}>CARPODRIVE</a>
        <a href="/register" style={{ fontSize: '12px', color: '#9E9890', textDecoration: 'none', fontFamily: sans }}>Crear cuenta</a>
      </div>

      <div className="login-layout" style={{ display: 'flex', flex: 1, flexDirection: 'column' }}>

        {/* Panel izquierdo — oculto en móvil */}
        <div className="login-left" style={{ width: '45%', background: '#1a1a1a', padding: '60px 56px', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden', display: 'none' }}>
        ...
        <div className="login-right" style={{ flex: 1, background: '#EDEDE9', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px' }}>
