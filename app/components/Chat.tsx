...
export default function Chat({ reserva_id, usuario_id, usuario_nombre, conductor_nombre, ruta, estadoReserva, onCerrar }: Props) {
  ...
  const chatActivo = estadoReserva === 'confirmada';
      ...
      <div style={{
        ...
        <div style={{ background: '#1a1a1a', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {!chatActivo && (
              <span style={{ fontSize: '11px', color: '#9E9890', fontFamily: sans, background: '#2e2e2e', padding: '4px 10px', borderRadius: '20px' }}>
                Chat cerrado
              </span>
            )}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {mensajes.length === 0 && (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              ...
              <p style={{ fontSize: '12px', color: '#D6CCC2', fontFamily: sans, marginTop: '4px' }}>
                {chatActivo ? 'Inicia la conversacion' : 'El viaje ya finalizó'}
              </p>
            </div>
          )}
          {mensajes.map(m => {
        ...
        {/* Input — bloqueado si el viaje no está confirmado */}
        {chatActivo ? (
          ...
          <div style={{ padding: '16px 20px', borderTop: '0.5px solid #EDEDE9', textAlign: 'center' }}>
            <p style={{ fontSize: '12px', color: '#9E9890', fontFamily: sans }}>
              {estadoReserva === 'completada' ? 'Este viaje ya fue completado.' : 'Esta reserva fue cancelada.'}
            </p>
          </div>
        )}
      </div>
